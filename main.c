...
size_t	load_images(t_data *data)
{
	...
	data->img_wall = mlx_xpm_file_to_image(data->mlx, "assets/wall.xpm",
			&data->tile_size, &data->tile_size);
	data->img_collectible = mlx_xpm_file_to_image(data->mlx, "assets/chest.xpm",
			&data->tile_size, &data->tile_size);
	data->img_player = mlx_xpm_file_to_image(data->mlx, "assets/player.xpm",
			&data->tile_size, &data->tile_size);
	data->img_exit = mlx_xpm_file_to_image(data->mlx, "assets/door.xpm",
			&data->tile_size, &data->tile_size);
	if (!(data->img_floor || data->img_wall || data->img_collectible
			|| data->img_player || data->img_exit))
		return (1);
	return (0);
}
...
int	main(int ac, char **av)
{
	...
	{
		printerr("Your file's name must end with .ber!");
		exit(1);
	}
	load_map(av[1], &data);
	check_map(&data);
